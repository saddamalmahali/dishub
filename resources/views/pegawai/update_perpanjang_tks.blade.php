    ...
    <div class="row">
        <div class="col-md-6 col-md-offset-3">
            <div class="panel panel-primary">
                ...
                <form action="{{url('/kepegawaian/update_perpanjang_tks')}}" method="post">
                    {{csrf_field()}}
                    ...
                    <div class="panel-body">
                        <div class="form-group{{ $errors->has('id_pegawai') ? ' has-error' : '' }}">
                            ...
                            <select name="id_pegawai" id="id_pegawai" class="form-control">
                                ...
                                @forelse ($data_pegawai as $pegawai)
                                    <option value="{{$pegawai->id}}" {{$tks->id_pegawai == $pegawai->id ? 'selected' : ''}}>{{$pegawai->nama_lengkap}}</option>
                                @empty
                        ...
                        <div class="form-group{{ $errors->has('no_sp') ? ' has-error' : '' }}">
                            <label>No SP</label>
                            <input type="text" name="no_sp" class="form-control" placeholder="Input Nomor SP" value="{{$tks->no_sp}}">
                            @if ($errors->has('no_sp'))
                                <span class="help-block">
                        ...
                        <div class="form-group{{ $errors->has('tanggal_sp') ? ' has-error' : '' }}">
                            ...
                            <input type="text" name="tanggal_sp" id="tanggal_sp" class="form-control" value="{{date('d-m-Y', strtotime($tks->tanggal_sp))}}" placeholder="Input Tanggal SP">
                        ...
                        <div class="form-group{{ $errors->has('tmt') ? ' has-error' : '' }}">
                            <label>TMT</label>
                            <input type="text" name="tmt" id="tmt" class="form-control" value="{{date('d-m-Y', strtotime($tks->tmt))}}" placeholder="Input Tanggal TMT">
                            @if ($errors->has('tmt'))
                                <span class="help-block">
